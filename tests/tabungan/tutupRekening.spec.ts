...
function tutupRekening() {
    ...
    test('testing Tutup Rekening', async ({ page }) => {
        ...
        await page.getByText('Tutup Rekening').isVisible;
        await page.getByText('Tutup Rekening').first().click();
        await page.getByTitle('PUSAT  | Tutup Rekening').isVisible;
        await page.screenshot({ path: 'Tutup Rekening.png', fullPage: true })
    })
