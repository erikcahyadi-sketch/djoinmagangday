...
function transferBankLain() {
    ...
    test('testing Perpanjang Tabungan', async ({ page }) => {
        ...
        await page.getByText('Transfer Bank Lain').first();
        await page.getByText('Transfer Bank Lain').first().click();
        await expect(page).toHaveTitle('PUSAT  | Transfer Keluar Bank Lain');
        await page.screenshot({ path: 'Transfer.png', fullPage: true })
    })
